...
ts_taglist * _ts_taglist_next(sds * next_tag, char mode) {
    // poorly named, but pushes a new item onto the linked-list of tags 
    ts_taglist * item = malloc(sizeof(ts_taglist));
    item->next = 0;
    item->operation = 0;

    // trim leading/trailing whitespace. If we're left with nothing, return
    sdstrim(*next_tag, " ");
...
ts_taglist * ts_taglist_create(sds tag_str) {
    ...
    for(int i = 0; i < tag_str_len; i++) {
        ...
        if(isspace(next_char)) {
            ...
            // but tags can have whitespace in the center
            ...
            // if the previous character is '~' (just check at i-1), 
            // remove all items from hash
            ...
            // build up a string that will be the next tag
